...
<body <?php body_class(); ?> data-lang="<?php echo esc_attr( $lang ); ?>" data-time="<?php echo esc_attr( $cal_time ); ?>" data-disweek="<?php echo esc_attr($calendar_dis_weekend); ?>">
    ...
    <div class="ovatheme_container_<?php echo esc_attr(ireca_get_current_width_site()); ?> <?php echo esc_attr($bg['bg_class']); ?> woocommerce">
		...
		<header class="ovatheme_header_version4 header_m <?php  echo esc_attr( $bg['bg_class'] ); ?>">
			<div class="container">
				<div class="header_content d-flex">
					...
					<div class="right">
						<!-- top -->
						<?php if( is_active_sidebar('header4_phone') || is_active_sidebar('header4_social') || is_active_sidebar('header4_cart') ){ ?>
							<div class="top d-none d-lg-block">
								<div class="container px-0">
									<div class="row">

										<?php if( is_active_sidebar('header4_phone') ){ ?>
											<div class="col text-left">
												<?php dynamic_sidebar('header4_phone'); ?>
											</div>
										<?php } ?>
										
										<?php if( is_active_sidebar('header4_social') ){ ?>
											<div class="col text-center">
												<?php dynamic_sidebar('header4_social'); ?>
											</div>
										<?php } ?>

										<?php if( is_active_sidebar('header4_cart') ){ ?>
											<div class="col col-last text-right">
												<?php dynamic_sidebar('header4_cart'); ?>
											</div>
										<?php } ?>
		...
		<?php if( $bg['bg_url'] ){ ?>
			<div class="header_img header_version4" style="background: url( <?php echo esc_url( $bg['bg_url'] ); ?> )" >
