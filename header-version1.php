...
<body <?php body_class(); ?> data-lang="<?php echo esc_attr( $lang ); ?>" data-time="<?php echo esc_attr( $cal_time ); ?>" data-disweek="<?php echo esc_attr($calendar_dis_weekend); ?>">
    ...
    <div class="ovatheme_container_<?php echo esc_attr(ireca_get_current_width_site()); ?> <?php echo esc_attr($bg['bg_class']); ?> woocommerce">
		...
		<header class="ovatheme_header_version1 header_m <?php  echo esc_attr( $bg['bg_class'] ); ?>">
			
			<!-- top -->
			<?php if( is_active_sidebar('header1_top_left') || is_active_sidebar('header1_top_center') || is_active_sidebar('header1_top_right') ){ ?>
				<div class="top d-none d-lg-block">
					<div class="container">
						<div class="row content_top">

							<?php if( is_active_sidebar('header1_top_left') ){ ?>
								<div class="col text-left">
									<?php dynamic_sidebar('header1_top_left'); ?>
								</div>
							<?php } ?>
							
							<?php if( is_active_sidebar('header1_top_center') ){ ?>
								<div class="col text-center">
									<?php dynamic_sidebar('header1_top_center'); ?>
								</div>
							<?php } ?>

							<?php if( is_active_sidebar('header1_top_right') ){ ?>
								<div class="col text-right">
									<?php dynamic_sidebar('header1_top_right'); ?>
								</div>
							<?php } ?>
			...
			<div class="middle d-none d-lg-block">
				<div class="container d-flex justify-content-between px-01">
					...
					<?php if( is_active_sidebar('header1_beside_logo') ){ ?>
					<div class="d-none d-lg-block">
						<div class="beside_logo d-flex">
							<?php dynamic_sidebar('header1_beside_logo'); ?>
						</div>	
					</div>
					<?php } ?>
					
					<?php if( is_active_sidebar('header1_cart_icon') ){ ?>
						<div class="float-sm-right d-none d-lg-block">
							<div class="cart d-flex">
								<?php dynamic_sidebar('header1_cart_icon'); ?>
							</div>
						</div>
					<?php } ?>
							...
							<nav class="navbar navbar-expand-lg pl-0 pr-0 py-0 px-0">
								...
								<div class="form-inline">
								  ...
								  <div class="collapse navbar-collapse" id="mainmenu">
								  	...
								  	<?php if( is_active_sidebar('header1_home_icon') ){ ?>
										<div class="form-inline header1_home_icon ml-lr-5 d-none d-lg-block">
											<?php dynamic_sidebar('header1_home_icon'); ?>
										</div>
									<?php } ?>
						            ...
						            <?php if( is_active_sidebar('header1_search') ){ ?>
										<div class="form-inline header1_search mx-0 ml-lg-5 d-none d-lg-block">
											<?php dynamic_sidebar('header1_search'); ?>
										</div>
									<?php } ?>
		...
		<?php if( $bg['bg_url'] ){ ?>
			<div class="header_img header_version1" style="background: url( <?php echo esc_url( $bg['bg_url'] ); ?> )" >
